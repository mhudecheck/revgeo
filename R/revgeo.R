...
revgeo <- function(longitude, latitude, provider = NULL, API = NULL, output = NULL, item = NULL) {
  ...
  ### Start Bing ###
    ...
    url <- paste0("http://dev.virtualearth.net/REST/v1/Locations/",latitude,",",longitude,"?o=json&key=",API)
    if(!(is.null(item))) {
    ...
    for(i in url) {
      print(paste0("Getting geocode data from Bing: ", i))
      data <- getURLAsynchronous(i)
      ...
      address_hash <- tryCatch(as.list(returned_data$resourceSets[[1]]$resources[[1]]$address), error=function(e) "House Number Not Found") 
      ...
      country <- tryCatch(address_hash$countryRegion, error=function(e) "Country Not Found")
      address_string <- tryCatch(address_hash$formattedAddress, error=function(e) "Address Number Not Found")
      if(is.null(street)) {
